...
export default function MarketInsightPanel({ data, last, prev, currency, symbol, label, sourceType }) {
  ...
  return (
    <section aria-labelledby="market-insight-heading">
      ...
      <div className="insight-sections" style={styles.sections}>
        ...
        <InfoSection title="주요 지표">
          <Metric label={<TermTooltip term="RSI">RSI (14)</TermTooltip>} value={formatDecimal1(last?.rsi)} hint={last?.rsi <= 30 ? '과매도' : last?.rsi >= 70 ? '과매수' : '중립'} />
          <Metric label={<TermTooltip term="MACD">MACD 히스토그램</TermTooltip>} value={formatDecimal2(last?.histogram)} hint={last?.histogram >= 0 ? '상승 모멘텀' : '하락 모멘텀'} />
          <Metric label={<TermTooltip term="ADX">ADX (14)</TermTooltip>} value={formatDecimal1(last?.adx)} hint={last?.adx >= 25 ? '추세 강함' : '추세 약함'} />
          <Metric label={<TermTooltip term="ATR">ATR (14)</TermTooltip>} value={formatPrice(last?.atr, currency)} />
          <Metric label={<TermTooltip term="EMA">EMA 20 / 50</TermTooltip>} value={last?.timing?.emaFast == null ? '—' : `${formatPrice(last.timing.emaFast, currency)} / ${formatPrice(last.timing.emaSlow, currency)}`} />
          <Metric label={<TermTooltip term="볼린저">볼린저 %B</TermTooltip>} value={last?.timing?.percentB == null ? '—' : `${(last.timing.percentB * 100).toFixed(1)}%`} />
        </InfoSection>
        ...
        <InfoSection title="거래현황">
          ...
          <Metric label={<TermTooltip term="상대거래량">상대거래량</TermTooltip>} value={last?.relVolume == null ? '—' : `${last.relVolume.toFixed(2)}x`} tone={last?.volumeConfirmed ? 'good' : 'warning'} />
...
function InfoSection({ title, children }) { return <div style={styles.section}><h3 style={styles.sectionTitle}>{title}</h3><div className="insight-grid" style={styles.grid}>{children}</div></div>; }
...
const styles = {
  ...
  label: { color: 'var(--text-muted)', fontSize: 10, overflow: 'visible', whiteSpace: 'nowrap' },
