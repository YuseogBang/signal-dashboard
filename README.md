...
- **모의투자 워크스페이스**: 실제 주문 없이 가상 매수·매도 체결, 현금·평가금액·실현/미실현손익,
  종목 비중 제한, 거래당 위험 제한, 일일 손실 한도, 손절·목표가 도달 경고를 제공
- **매매일지**: 거래 이유와 판단을 브라우저에 저장하고 최근 체결·일지를 다시 확인
- **실시간 모니터링**: 실시간 조회 종목은 사용자가 선택하면 1분 주기로 자동 갱신하며, 최신 가격을
  포지션 평가와 손절·목표가 모니터링에 반영
...
- **상태형 UI**: 상승은 그린/틸, 하락은 블루 계열의 그라데이션으로 표현하고 대회 모드에서는 판단에 필요한
  요약·타이밍·차트·모의 포지션만 남겨 화면 밀도를 줄임
- **포트폴리오 제출 모드**: 데모 모드, 데이터 출처·신뢰도 배지, 시그널 긍정·주의 요인, 방법론·데이터 한계 안내를
  화면에 노출해 심사자가 API 키 없이도 제품의 문제 정의와 설계 의도를 확인할 수 있음
...
이 프로젝트에서 REST API는 포트폴리오의 핵심 구현 증거입니다. 프론트엔드가 브로커 API를 직접 호출하지 않고,
서버 측 어댑터가 인증·파라미터 검증·응답 정규화를 담당합니다. 현재 공개 데모는 토스증권 캔들 API를 예시
공급자로 사용하고, 분석 계층은 NH선물 REST API 같은 다른 브로커로 교체할 수 있도록 공통 OHLCV 구조를 사용합니다.

NH선물 REST API는 HTS의 `2043 API 서비스 사용등록`과 모의거래 테스트가 필요한 별도 공급자입니다. 실제 NH 엔드포인트를
추측해 넣지 않고, 개발 문서와 모의 API 인증키가 확보되면 공급자 어댑터만 연결하는 방식으로 확장합니다.

## 실시간 데이터 연동 (토스증권 캔들 API 예시)
...
지원서에서 문제 정의, 분석 설계, 브로커 REST API 연동 구조, 백테스트와 리스크 관리 결과를 설명할 때는
